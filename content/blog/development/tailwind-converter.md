...
즉 코드를 통해 생성된 추상 구문 트리를 조작하면 코드가 재생성될 때 원하는 결과를 얻을 수 있을 것이다. 사실 매우 복잡한 추상 구문 트리를 직접 조작하기란 절대 쉽지
않다. 그래도 가장 안전한 방법이라는 생각이 들었다. **추상 구문 트리의 조작이 유효하지 않다면 에러가 발생하게 되는데, 인위적인 조작이 유효한지 프로그래밍적으로 
